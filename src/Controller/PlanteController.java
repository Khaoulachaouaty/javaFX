...
public class PlanteController {
    ...
    private int currentIndex = 2; // Indice de la plante actuellement affichée

    public void initialize() {
        try {
            Connection conn = Connexion.getCn();
            String query = "SELECT * FROM plante WHERE id = ?";
        ...
        /*fav.setOnAction(event -> {
            if (fav.isSelected()) {
                mettreAJourFavori(true); // Appeler la fonction de mise à jour pour définir "favori" à "oui"
            } else {
                mettreAJourFavori(false); // Appeler la fonction de mise à jour pour définir "favori" à "non"
            }
        });*/
    }

    @FXML
    private void afficherPlanteSuivante() {
        try {
            Connection conn = Connexion.getCn();
            String query = "SELECT * FROM plante WHERE id = ?";
            PreparedStatement stmt = conn.prepareStatement(query);
        ...
        currentIndex++; // Incrémenter l'indice pour la prochaine plante
    }

    @FXML
    private void retourPlantePrecedente() {
        if (currentIndex > 1) {
            currentIndex -= 2; // Décrémenter l'indice pour afficher la plante précédente
            afficherPlanteSuivante();
        }
    }

    private void afficherPlante(Plante plante) {
        System.out.println("Affichage des données de la plante :");
        System.out.println("Nom : " + plante.getNom());
        System.out.println("Catégorie : " + plante.getCategorie());
        System.out.println("Sel minimum : " + plante.getSelMin());
        System.out.println("Virus : " + plante.getVirus());
        System.out.println("Saison : " + plante.getSaison());
        System.out.println("Arrosage : " + plante.getArrosage());
        System.out.println("Quantité d'eau : " + plante.getQteEau());
        System.out.println("Description : " + plante.getDescription());
        System.out.println("Florison : " + plante.getFlorison());
